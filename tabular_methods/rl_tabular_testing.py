...
class AlgorithmsTesting:
    ...
    def test_mc_non_exploring_starts_control(episodes, print_q_table_and_policy):
        ...
        utils.Plotter.plot_running_average(frozen_lake_env.name, method_name, frozen_lake_scores,
                                   window=episodes//100, show=True)
        utils.Plotter.plot_accumulated_scores(frozen_lake_env.name, method_name, frozen_lake_accumulated_scores, show=True)
        frozen_lake_scores, frozen_lake_accumulated_scores = utils.Tester.test_policy(frozen_lake_env, frozen_lake_policy)
        utils.Plotter.plot_running_average(frozen_lake_env.name, method_name, frozen_lake_scores,
                                   window=episodes//100, show=True)
        utils.Plotter.plot_accumulated_scores(frozen_lake_env.name, method_name, frozen_lake_accumulated_scores, show=True)
        ...
        utils.Plotter.plot_accumulated_scores(blackjack_env.name, method_name, blackjack_accumulated_scores, show=True)
        blackjack_accumulated_scores = utils.Tester.test_policy(blackjack_env, blackjack_policy)
        utils.Plotter.plot_accumulated_scores(blackjack_env.name, method_name, blackjack_accumulated_scores, show=True)
    ...
    def test_sarsa():
        ...
        utils.Plotter.plot_running_average(taxi_env.name, method_name, taxi_scores, show=True)
        taxi_scores = utils.Tester.test_q_table(taxi_env, taxi_Q_table)
        utils.Plotter.plot_running_average(taxi_env.name, method_name, taxi_scores, show=True)
        ...
        utils.Plotter.plot_running_average(mountain_car_env.name, method_name, mountain_car_scores, show=True)
        mountain_car_scores = utils.Tester.test_q_table(mountain_car_env, mountain_car_Q_table)
        utils.Plotter.plot_running_average(mountain_car_env.name, method_name, mountain_car_scores, show=True)
        ...
        utils.Plotter.plot_running_average(cart_pole_env.name, method_name, cart_pole_scores, show=True)
        cart_pole_scores = utils.Tester.test_q_table(cart_pole_env, cart_pole_Q_table)
        utils.Plotter.plot_running_average(cart_pole_env.name, method_name, cart_pole_scores, show=True)
        ...
        utils.Plotter.plot_running_average(acrobot_env.name, method_name, acrobot_scores, show=True)
        acrobot_scores = utils.Tester.test_q_table(acrobot_env, acrobot_Q_table)
        utils.Plotter.plot_running_average(acrobot_env.name, method_name, acrobot_scores, show=True)
    ...
    def test_double_q_learning():
        ...
        utils.Plotter.plot_running_average(taxi_env.name, method_name, taxi_scores, show=True)
        taxi_Q1_scores = utils.Tester.test_q_table(taxi_env, taxi_Q1_table)
        taxi_Q2_scores = utils.Tester.test_q_table(taxi_env, taxi_Q2_table)
        scores_list = [taxi_Q1_scores, taxi_Q2_scores]
        labels = ['Q1', 'Q2']
        utils.Plotter.plot_running_average_comparison(taxi_env.name + ' - ' + method_name, scores_list, labels, show=True)
        ...
        utils.Plotter.plot_running_average(mountain_car_env.name, method_name, mountain_car_scores, show=True)
        mountain_car_Q1_scores = utils.Tester.test_q_table(mountain_car_env, mountain_car_Q1_table)
        mountain_car_Q2_scores = utils.Tester.test_q_table(mountain_car_env, mountain_car_Q2_table)
        scores_list = [mountain_car_Q1_scores, mountain_car_Q2_scores]
        labels = ['Q1', 'Q2']
        utils.Plotter.plot_running_average_comparison(mountain_car_env.name + ' - ' + method_name, scores_list, labels, show=True)
        ...
        utils.Plotter.plot_running_average(cart_pole_env.name, method_name, cart_pole_scores, show=True)
        cart_pole_Q1_scores = utils.Tester.test_q_table(cart_pole_env, cart_pole_Q1_table)
        cart_pole_Q2_scores = utils.Tester.test_q_table(cart_pole_env, cart_pole_Q2_table)
        scores_list = [cart_pole_Q1_scores, cart_pole_Q2_scores]
        labels = ['Q1', 'Q2']
        utils.Plotter.plot_running_average_comparison(cart_pole_env.name + ' - ' + method_name, scores_list, labels, show=True)
...
class EnvironmentsTesting:
    ...
    def test_environment(env, episodes, eps_max=1.0, eps_dec=None, alpha=0.1,
                         q_table_test_method=utils.Tester.test_q_table,
                         policy_test_method=utils.Tester.test_policy,
        ...
        if show_scores:
            ...
            utils.Plotter.plot_running_average_comparison(env.name + ' - Training', scores_list, labels,  # window=episodes//100,
        ...
        if show_accumulated_scores:
            ...
            utils.Plotter.plot_accumulated_scores_comparison(env.name + ' - Training', accumulated_scores_list, labels,
        ...
        if show_scores:
            scores_list = [scores_mc_01, scores_mc_02, scores_sarsa, scores_e_sarsa, scores_q_l, scores_d_q_l]
            utils.Plotter.plot_running_average_comparison(env.name + ' - Test', scores_list, labels,  # window=episodes//100,
        ...
        if show_accumulated_scores:
            accumulated_scores_list = [accumulated_scores_mc_01, accumulated_scores_mc_02,
                                       accumulated_scores_sarsa, accumulated_scores_e_sarsa,
                                       accumulated_scores_q_l, accumulated_scores_d_q_l]
            utils.Plotter.plot_accumulated_scores_comparison(env.name + ' - Test', accumulated_scores_list, labels,
