...
class Atari:
    ...
    def stack_frames(stacked_frames, frame):  # to get a sense of motion. observation == frame, prev s == stacked_frames
        if stacked_frames is None:  # start of the episode: duplicate frame
            return np.repeat(frame, repeats=Atari.frames_stack_size, axis=2)
    ...
    class Breakout(BaseEnv):
        ...
        def __init__(self):
            ...
            self.image_channels = Atari.IMAGE_CHANNELS_GRAYSCALE
            self.relevant_screen_size = (180, 160)
            self.input_dims = (*self.relevant_screen_size, Atari.frames_stack_size)    # Box(210,160,3)
        ...
        def get_state(self, observation, prev_s):
            ...
            s = Atari.stack_frames(prev_s, observation_pre_processed)
            return s

        def preprocess(self, observation):
            cropped_observation = observation[30:, :]
            if self.image_channels == Atari.IMAGE_CHANNELS_RGB:
    ...
    class SpaceInvaders(BaseEnv):
        ...
        def get_state(self, observation, prev_s):
            observation_pre_processed = self.preprocess(observation)
            s = Atari.stack_frames(prev_s, observation_pre_processed)  # here it should be 3 frames?
            return s
